import { Hume, MimeType, convertBase64ToBlob, getBrowserSupportedMimeType } from 'hume';
...
type ScriptReaderOutType = [(line: string, prevLine: string) => Promise<void>, () => void];
...
export const getScriptReader = async (
  ...
  await socket.sendSessionSettings({
    systemPrompt: `The setting is ${setting}.` + LINE_READING_SYSTEM_INSTRUCTION
  });

  const readLine = (line: string, prevLine: string) => new Promise<void>((resolve) => {
    finishCallback = () => {
      resolve();
    };
